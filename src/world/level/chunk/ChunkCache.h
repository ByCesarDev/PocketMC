...
public:
    ...
    bool fits(int x, int z) {
        return (x >= CHUNK_CACHE_MIN && z >= CHUNK_CACHE_MIN && x <= CHUNK_CACHE_MAX && z <= CHUNK_CACHE_MAX);
    }
    ...
    std::string gatherStats() {
        int total = CHUNK_CACHE_WIDTH * CHUNK_CACHE_WIDTH;
        char buf[64];
        sprintf(buf, "ChunkCache: %d", total);

	...
	bool place(Level* level, Random* random, int x, int y, int z) {
        ...
        for (int hh = 0; hh < treeHeight; hh++) {
            ...
            if (t == 0 || t == ((Tile*)Tile::leaves)->id) {
                int placeId = Tile::treeTrunk->id;
                int dataToPlace = trunkType;
                if (trunkType == TreeTile::BIRCH_TRUNK) {
                    placeId = Tile::birchTrunk->id;
                    dataToPlace = 0;
                } else if (trunkType == TreeTile::DARK_TRUNK) {
                    placeId = Tile::spruceTrunk->id;
                    dataToPlace = 0;
                }
                placeBlock(level, x, y + hh, z, placeId, dataToPlace);
            }
        }
        return true;
    }

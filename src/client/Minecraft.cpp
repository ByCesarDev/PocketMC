...
void Minecraft::tickInput() {
	...
	while (Mouse::next()) {
		...
		if (allowGuiClicks && e.action == MouseAction::ACTION_RIGHT && e.data == MouseAction::DATA_DOWN) {
			gui.handleClick(MouseAction::ACTION_RIGHT, Mouse::getX(), Mouse::getY());
		}
	...
	while (Keyboard::next()) {
		...
		if (isPressed) {
			...
			if (key == Keyboard::KEY_F4) {
				showEntityHitboxes = !showEntityHitboxes;
			}

			if (key == Keyboard::KEY_F5) {
				int view = options.getIntValue(OPTIONS_THIRD_PERSON_VIEW);
				options.set(OPTIONS_THIRD_PERSON_VIEW, (view + 1) % 3);
				/*
				ImprovedNoise noise;
				for (int i = 0; i < 16; ++i)
					printf("%d\t%f\n", i, noise.grad2(i, 3, 8));
...
void Minecraft::optionUpdated(OptionId option, bool value ) {
	if(netCallback != NULL && option == OPTIONS_SERVER_VISIBLE) {
		...
		_reloadInput();
	} else if (option == OPTIONS_AMBIENT_OCCLUSION || option == OPTIONS_SMOOTH_LIGHTNING) {
		useAmbientOcclusion =
			options.getBooleanValue(OPTIONS_AMBIENT_OCCLUSION) ||
			options.getBooleanValue(OPTIONS_SMOOTH_LIGHTNING);

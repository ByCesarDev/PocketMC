...
void ItemRenderer::renderGuiItemCorrect(Font* font, Textures* textures, const ItemInstance* item, int x, int y) {
		...
		TileRenderer::canRender(Tile::tiles[item->id]->getRenderShape()))
	{
		...
		if (tile) {
			for (int f = 0; f < 6; ++f) {
				int tface = tile->getTexture(f, item->getAuxValue());
				if (tface & Tile::TEXTURE_ALT_FLAG) { hasAlt = true; break; }
			}
		}

		// Prepare depth + clear (done once)

		if (!depthWasEnabled) glEnable2(GL_DEPTH_TEST);
		if (!depthWriteMaskWasEnabled) glDepthMask(GL_TRUE);
		...
		// First pass: main atlas
		textures->loadAndBindTexture("terrain.png");
		tileRenderer->setAtlasFilter(0);
		glScalef2(1, 1, 1);
		tileRenderer->renderGuiTile(tile, item->getAuxValue());

		// Second pass: alternate atlas (only if needed).
		// Use LEQUAL so fragments with equal depth pass (GUI cube geometry is identical).
		if (hasAlt) {

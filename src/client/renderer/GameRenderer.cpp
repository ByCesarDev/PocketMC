...
void GameRenderer::setupCamera(float a, int eye) {
    // Reset depth state every frame to avoid state leaks from GUI/entity passes.
...
void GameRenderer::renderLevel(float a) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if(mc->options.getBooleanValue(OPTIONS_FANCY_GRAPHICS)) {
            ...
            // Sky should not write depth, otherwise sun/moon can cover distant terrain/water.
...
void GameRenderer::moveCameraToPlayer(float a) {
	...
	if(player->isPlayer() && ((Player*)player)->isSleeping()) {
        ...
        if (mc->options.getBooleanValue(OPTIONS_FIXED_CAMERA)) {
            ...
            if (mc->options.getIntValue(OPTIONS_THIRD_PERSON_VIEW) == 2) {
                xd = -xd;
                zd = -zd;
                yd = -yd;
            }
...
bool GameRenderer::updateFreeformPickDirection(float a, Vec3& outDir) {
    ...
    bool firstPerson = mc->options.getIntValue(OPTIONS_THIRD_PERSON_VIEW) == 0;
...
void GameRenderer::setupClearColor(float a) {
    if (mc->level == NULL || mc->cameraTargetPlayer == NULL) {
    ...
    float* sunriseColor = level->dimension->getSunriseColor(level->getTimeOfDay(a), a);
    if (sunriseColor != NULL) {
        float mix = sunriseColor[3];
        fr = fr * (1.0f - mix) + sunriseColor[0] * mix;
        fg = fg * (1.0f - mix) + sunriseColor[1] * mix;
        fb = fb * (1.0f - mix) + sunriseColor[2] * mix;
    }

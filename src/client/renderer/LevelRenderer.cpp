...
void LevelRenderer::_renderSunOrMoon(float alpha, bool moon) {
	...
	if (moon && texData && texData->w == 128 && texData->h == 64) {
		u1 = 0.25f;
		v1 = 0.5f;
	}
	...
	if (moon) {
		// Compatibility: if moon texture is still the 128x64 atlas, use first 32x32 tile.
...
void LevelRenderer::takePicture( TripodCamera* cam, Entity* entity )
{
	...
	mc->options.set(OPTIONS_THIRD_PERSON_VIEW, 0);

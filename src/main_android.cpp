...
// This file is compiled separately; do not include main.cpp here.
...
static void setupExternalPath(struct android_app* state, Minecraft* app)
{
    ...
    while( 1 )
    {
        ...
        while( (ident = ALooper_pollOnce( 0, NULL, &events, (void**)&source) ) >= 0 )
        {
		 ...
		 if (!inited && engine.is_inited) {
			 ((App*)app)->init(engine.appContext);

...
elif command -v javac.exe >/dev/null 2>&1; then
  JAVAC_CMD="$(command -v javac.exe)"
elif [[ -n "$JAVA_HOME_DEFAULT" && -x "$JAVA_HOME_DEFAULT/bin/javac" ]]; then
  JAVAC_CMD="$JAVA_HOME_DEFAULT/bin/javac"
elif [[ -n "$JAVA_HOME_DEFAULT" && -x "$JAVA_HOME_DEFAULT/bin/javac.exe" ]]; then
  JAVAC_CMD="$JAVA_HOME_DEFAULT/bin/javac.exe"
...
function find_keytool() {
  ...
  if [[ -n "$JAVA_HOME_DEFAULT" && -x "$JAVA_HOME_DEFAULT/bin/keytool.exe" ]]; then
    echo "$JAVA_HOME_DEFAULT/bin/keytool.exe"
    return
  fi
